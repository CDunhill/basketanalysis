...
The *apriori* function is used to generate the rules for the data set. The *support* has been set to 0.0015 (0.15%). There are about 51,000 transactions in our data so a support of 1.5% means that any rules with fewer than 77 transactions will be ignored. The *confidence* limit has been set to 0.5 (50%) so only rules which are proven to be true in more than half the supported transactions will be shown. The *maxlen* parameter limits the number of basket items in the calculation [... I think!]

```{r rules, message=FALSE, warning=FALSE}
rules <- apriori(mwlw, parameter = list(supp = 0.0015, conf = 0.5,maxlen=5))
...
```{r viz}
plot(rules, method = "graph", igraphLayout = "layout_with_fr")
# Check ??layout_in_circle for other options
```
